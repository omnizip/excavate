...
- **reverted** — completed, then undone by a later review; see the file
...
| 07 | [07-archive-targets-helper.md](07-archive-targets-helper.md) | P2 | reverted | Moved target-path safety into `Excavate::Targets`, then inlined it back into `Archive` as private methods (25412f1). |
...
| 11 | [11-specs-cover-new-collaborators.md](11-specs-cover-new-collaborators.md) | P2 | done | Add specs for the new `Filesystem`, `Selection`, and `NestedCabFallback` collaborators introduced by 06–09. Target-path policy is covered through `Archive#extract`. |
...
### Updated
- `archive_spec.rb` — added a "target path policy" context covering the existence, emptiness, and default-target rules through `Archive#extract`
